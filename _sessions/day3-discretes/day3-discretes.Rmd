...
2) Compare Bayesian models using LOO [not convincingly better]
...
$$\lambda = e^{\alpha}*e^{\beta_1 Discharge}$$
...
Slope estimates for each treatment (fold-change)

```{r eval=T, include=T, fig.height=3, fig.width=5}
  library(emmeans)
  emtrends(mod_gp, ~1, var='discharge_gl') 
...
```{r eval=T, include=T, fig.height=3, fig.width=5}
  library(emmeans)
  emtrends(mod_mc, ~works, var='year') %>% data.frame() %>% 
    ggplot( aes(works, y= exp(year.trend), ymin= exp(lower.HPD), ymax= exp(upper.HPD) ) ) +
      geom_hline(yintercept=1, lty=2) +
      geom_pointrange() + labs(x='Treatment', y='Year-fold change')
  
```

Pairwise contrasts between slopes (on log-scale)...

```{r eval=T, include=T, fig.height=3, fig.width=5}
  library(emmeans)
  emtrends(mod_mc, pairwise~works, var='year')$contrast %>% data.frame() 
```
