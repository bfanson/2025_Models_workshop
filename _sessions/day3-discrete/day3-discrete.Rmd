...
- Adapt Day 2's workflow to discrete
  - Introduce visualisation approaches helpful for discrete data
  - link functions when working binomial, Poisson, negative binomial
  - Discuss interpretation

# Project to download

[Link](https://delwpvicgovau-my.sharepoint.com/:u:/g/personal/ben_fanson_deeca_vic_gov_au/EabXXTmx9_pHsSpUepRIViIBVyTLKjmt8LM6B3jBwFn4Kg?e=dzggg9)

# Key packages

![](images/tidyverse.png){width="10%"}![](images/ggplot2.png){width="10%"}![](images/brms.png){width="10%"}![](images/broom.png){width="10%"} 

# Review of analysis workflow so far
...
3) modeling time
    - model formulation ( y ~ sex + habitat )
...
1) Most of the distribution (not LogNormal) we are talking about come from the same family, __the Exponential family__
    -  this means they can all be rewritten as this ugly (but beautiful) forumula: (x|θ) = h(x) exp{η(θ)ᵀT(x) - A(θ)}
...
3) All part of converting the distribution into the exponential family notion, there are what is called __link__ functions.  Table below has the "natural" link function 

__note -__ natural as it has the best statistical properties.  You can use other link functions
...
Researchers were interested in improving detection of skink during surveys when searching a plot, allowing for better areas with She-oak populations.  The logic for the design is that placing out concrete roof tiles provide a thermal microhabitat and does not require disturbing natural shelters.  As tiles heat up by solar radiation, skinks are more likely to use them for thermoregulation.

```{r  eval=T, echo=T, fig.height=3, fig.width=8,  dpi=650}
library(cowplot)
cowplot::plot_grid(
...
Here, we are using a simplified design of original project (and simulated data).  A total of `r n_tiles` were placed in `r n_area` spatial areas.   The checking of tiles was over several time periods (but that data is not included here).  When a tile was checked, the tile temperature (continuous) underneath was measured using a infrared thermometer, as well if it was a sunny day (binary: yes/no).

```{r eval=T, fig.height=3, fig.width=8,  dpi=650}
...
__Recommendation:__ I always make a visual of the design. This example is a bit simple but will still do...
...
```{r eval=F, echo=F, fig.height=3, fig.width=8}
plot_grid(
...
Each tile is a sampling event and each sampling event has unique attributes (temp).  So, I would keep as N=1.  Since we have a binary response (0/1) and N=1, I am thinking Bernoulli distribution here (__note-you could use Binomial as well but selecting Bernoulli is more efficient__)
...
$$
\text{logit}(p_i) = \beta_0 + \beta_1 * \text{sunny}_i + \beta_2 * \text{temperature}_i +
\beta_3 * \text{area}_i
$$

where _i_ is observation _i_; $\beta_1$ and $\beta_3$ take either 0/1 (indicator variables)


__Link function__

log odds = logit(_p_) = $log(\frac{p}{(1-p)})$ where odds= \frac{p}{(1-p)}


In words, the odds ($\frac{p}{(1-p)}$) is of probability of success relative to it not happening.  Logging turns transforms to additive scale which is how we add things in our equations
...
__NOTE: This means that the parameter estimates from model will be on the logit scale__
...
runOnce <- function(){
  library(brms)
  mod_skink <- brm( detected ~ sunny  + tile_c + area, data = ds_skink, family = 'Bernoulli', cores = 4)
    saveRDS(mod_skink,'data/mod_skink.rds')
  mod_skink2 <- brm( detected ~ sunny  + s(tile_c, by =sunny)  + area, 
                     data = ds_skink , family = 'Bernoulli', cores = 4, 
...
```{r eval=T, echo=T,fig.height=3, fig.width=6}
  ...
  ggplot(ds_skink, aes(tile_c, detected, col=sunny) ) + 
    ...
    geom_line( aes(y=fit ) ) + 
    geom_line( aes(y=fit2 ), lty=2 ) + 
      labs( x='Tile temperature')
...
```{r eval=T, echo=T}
  loo_compare(loo( mod_skink), loo(mod_skink2, moment_match = T) )
```
...
\mu_i &= \beta_0 + \beta_1 t_i + \beta_2 v_i + \beta_3 t_i v_i + \epsilon_{transect_i} \\
...
Let's get an idea of the catch rate variable across the fishways...
...
$$
\begin{align}
log(\lambda_i) &= \beta_0 + \beta_1 Discharge + \epsilon_{month_i} + \epsilon_{fishway_i} \\
...
$$\lambda = e^{\beta_0}*e^{\beta_1Discharge}$$
apply link function (log)...

$$log(\lambda) = log(left(\frac{n}{\text{effort}}\right) = \beta_0 + \beta_1 Discharge)$$
$$\log\left(\frac{n}{\text{effort}}\right) = \log(n) - \log(\text{effort})$$

$$log(\lambda) = log(\frac{n}{\text{effort}}) =  \log(n) - \log(\text{effort})=\beta_0 + \beta_1 Discharge$$
$$log(\lambda) = \beta_0 + \beta_1 Discharge + \log(\text{effort})$$
...
__Table: Summary of coefficients__
```{r eval=T, echo=T}
# ds_tbl <- broom.mixed::tidy(mod_gp)
# evid <- (ds_tbl$conf.low*ds_tbl$conf.high > 0) & (ds_tbl$effect == 'fixed')
# ds_tbl<- ds_tbl %>%
#           select(-component) %>%
...
$$
\begin{align}
log(\mu_i) &= \beta_0 + \beta_1 year + \beta_2 works + \beta_3 (year*works) + \epsilon_{reach_i} \\
